...
for i=1:length(alphas)
   ...
   [ EP,OJ(l-1),utheta,OJS] = WAS_OptCont_Euler(T,h,maxTimes,A,theta_init,alpha,beta,c1,c2);
    ...
    filename = ['test\test_Element_Alpha_SW',t,'.xls'];
...
ylabel('OJ');
saveas(gcf,['Fig_OnNet\alpha_SW',t,'.jpg']);
end

%%
Igor = {length(alphas)+1,4};
Igor{1,1} = 'alpha';
Igor{1,2} = 'SW_OJ_alpha';
Igor{1,3}='beta';
Igor{1,4}=beta;
Igor{1,5}='c1';
Igor{1,6}=c1;
...
filename = ['Results_OnExcels\alpha_SW',t,'.xls'];
xlswrite(filename,Igor);
