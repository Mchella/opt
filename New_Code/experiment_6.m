function eNumReturn=experiment_6()
clc
clear
...
%xlswrite('results\experiment6.xlsx', eNumReturn);
% subplot(2,3,1)
    plot([2:2:20],eNumReturn,'ro-')
    set(gcf,'color','w')  
    xlabel('$\mu$','interpreter','latex')
    %xlabel('$\overline \theta$');
    ylabel('$J(\theta_{\mu}^*)$','interpreter','latex')
    %ylabel('J(\theta^{*}_{T})')
    set(gca,'FontSize',19);
    %set(gca,'FontSize',20);
    print fig\experiment_6.eps -depsc2 -r600
%    subplot(1,3,2)
%    plot(I)
%    subplot(2,3,3)
%    plot(U)
