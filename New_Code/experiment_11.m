function eNumReturn=experiment_11()
clc
clear
...
for t=1:n-1
    ...
    I(t+1)=I(t)+h*dI;
    dU=alpha*I(t)*W(t)-beta_2(theta(t))*U(t)+gamma*W(t)-delta*U(t);
    U(t+1)=U(t)+h*dU;
    dW=beta_2(theta(t))*U(t)-gamma*W(t)-delta*W(t);
    W(t+1)=W(t)+h*dW;
        if(I(t+1)<0)
            I(t+1)=0;
        end
...
for tt=1:maxTime
    ...
    for t=n:-1:2
        ...
        dLambda_2=(beta_2(theta(t))+delta)*lambda_2(t)-beta_2(theta(t))*lambda_3(t)-omega_2;
    ...
    for t=1:n
        ...
        for iter=1:length(Theta)
            ...
            pf = (lambda_3(t)-lambda_2(t))*U(t)*beta_2(Theta(iter))-W(t)*Theta(iter);
            if pf>=optPf
                optPf = pf;
                theta(t) = Theta(iter);
            end
...
%xlswrite('results\experiment9.xlsx', eNumReturn);

    plot([0.1:0.1:1],eNumReturn,'ro-')
    set(gcf,'color','w')  
    xlabel('$\omega_2$','interpreter','latex')
    %xlabel('$\overline \theta$');
    ylabel('$J(\theta_{\omega_2}^*)$','interpreter','latex')
    %ylabel('J(\theta^{*}_{T})')
    set(gca,'FontSize',19);
    %set(gca,'FontSize',20);
    print fig\experiment_11.eps -depsc2 -r600
%saveas(gcf,'fig\experiment_11.eps')
